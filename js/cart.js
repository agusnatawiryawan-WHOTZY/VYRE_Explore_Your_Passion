document.addEventListener("DOMContentLoaded", () => {
  ...
  const render = () => {
    const cart = readCart();
    if (!cart.length) {
      content.innerHTML = '<div class="emptyCart"><h2>Your cart is empty</h2><p>Choose something that feels like you.</p><a class="shopButton" href="index.html">Explore products</a></div>';
      return;
    }
  ...
  content.addEventListener("click", (event) => {
    ...
    if (control.dataset.action === "decrease") cart[index].quantity = Math.max(1, cart[index].quantity - 1);
    if (control.dataset.action === "remove") { cart.splice(index, 1); showToast("Product removed from cart."); }
    saveCart(cart);
    render();
  });
  render();
