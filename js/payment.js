document.addEventListener("DOMContentLoaded", () => {
  ...
  const closeSuccess = () => { modal.classList.remove("is-open"); modal.setAttribute("aria-hidden", "true"); };
  if (!cart.length) {
    content.innerHTML = '<div class="emptyCart"><h2>No items to pay for</h2><p>Add a product to your cart first.</p><a class="shopButton" href="index.html">Explore products</a></div>';
    return;
  }
  ...
  document.querySelectorAll("[name='method']").forEach((method) => method.addEventListener("change", () => {
    const isCard = document.querySelector("[name='method']:checked").value === "card";
