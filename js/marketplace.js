document.addEventListener("DOMContentLoaded", () => {
  ...
  if (searchArea && marketHeader) marketHeader.insertBefore(searchArea, menuToggle);
  if (marketHeader && !marketHeader.querySelector(".marketUtility")) {
    const utilityBar = document.createElement("div");
    utilityBar.className = "marketUtility";
    utilityBar.innerHTML = "<span>VYRE MARKETPLACE</span><span>Gear untuk setiap versi dirimu</span>";
    marketHeader.prepend(utilityBar);
  }
  const currentUser = JSON.parse(localStorage.getItem("vyreCurrentUser") || "null");
  if (currentUser) document.querySelector("#marketProfileLabel").textContent = currentUser.name;
  ...
  const products = [
    ["FASHION", "VYRE Utility Jacket", "assets/vyreLogo-removebg-preview.png", 899000],
    ["FASHION", "Motion Essential Tee", "assets/vyreLogo-removebg-preview.png", 299000],
  ...
  const updateCount = () => {
    ...
    document.querySelector("#marketCartCount").textContent = cart.reduce((sum, item) => sum + item.quantity, 0);
  };
  const addToCart = (product) => {
    if (!currentUser) { window.location.href = `auth.html?redirect=${encodeURIComponent("marketplace.html")}`; return; }
    const cart = JSON.parse(localStorage.getItem("vyreCart") || "[]");
  ...
  const render = () => {
    ...
    let visibleProducts = products.filter((product) => (activeFilter === "ALL" || product[0] === activeFilter) && (!query || product[1].toLowerCase().includes(query) || product[0].toLowerCase().includes(query)));
    if (sortSelect.value === "low") visibleProducts.sort((a, b) => a[3] - b[3]);
    if (sortSelect.value === "high") visibleProducts.sort((a, b) => b[3] - a[3]);
    if (sortSelect.value === "name") visibleProducts.sort((a, b) => a[1].localeCompare(b[1]));
    resultCount.textContent = `${visibleProducts.length} PRODUK`;
    grid.innerHTML = visibleProducts.length ? visibleProducts.map((product) => `<article class="catalogCard"><div class="catalogCardImage"><span class="marketBadge">${product[0]}</span><img src="${product[2]}" alt="${product[1]}" loading="lazy"></div><div class="catalogCardInfo"><p class="catalogCardCategory">${product[0]}</p><h2>${product[1]}</h2><div class="marketRating"><span>★★★★★</span> 4.9 · 128 terjual</div><div class="catalogCardBottom"><strong class="catalogCardPrice">${formatPrice(product[3])}</strong><button class="catalogAdd" type="button" data-product-index="${products.indexOf(product)}" aria-label="Tambah ${product[1]}"><span class="material-symbols-rounded">add_shopping_cart</span></button></div></div></article>`).join("") : `<p class="catalogEmpty">Produk yang kamu cari belum ditemukan.</p>`;
  };
  document.querySelectorAll(".catalogFilter").forEach((button) => button.addEventListener("click", () => {
    document.querySelectorAll(".catalogFilter").forEach((item) => item.classList.remove("is-active"));
  ...
  document.querySelector("#marketSearchForm").addEventListener("submit", (event) => event.preventDefault());
  sortSelect.addEventListener("change", render);
  document.querySelectorAll("[data-search-term]").forEach((button) => button.addEventListener("click", () => { searchInput.value = button.dataset.searchTerm; render(); searchInput.focus(); }));
  const marketNav = document.querySelector("#marketNav");
  menuToggle.addEventListener("click", () => { const open = marketNav.classList.toggle("is-open"); menuToggle.setAttribute("aria-expanded", String(open)); menuToggle.querySelector(".material-symbols-rounded").textContent = open ? "close" : "menu"; });
  updateCount();
  render();
});
